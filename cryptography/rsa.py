...
prime_one = 13
prime_two = 7
n = prime_one * prime_two  # Max number that can be encrypted
...
def encrypt(number, pub_key, n):
    print(f"original number: {number}")

    # encrypted_number = pow(number, pub_key, n)
    encrypted_number = (number**pub_key) % 91
    print(f"encrypted number is: {encrypted_number} ")
    return encrypted_number


def decrypt(encrypted_number, private_key, n):
    decrypted_number = (encrypted_number**private_key) % 91
    print(f"decrypted number is: {decrypted_number}")
    return decrypted_number


encrypted_number = encrypt(4, pub_key, n)
descrypted_number = decrypt(encrypted_number, private_key, n)

text = "CLOUD"
numbers = [ord(char) for char in text]
print("UTF-8 representation of CLOUD:", numbers)
encrypted_message = [encrypt(num, pub_key, n) for num in numbers]
print("Encrypted Message:", encrypted_message)

decrypted_message = [decrypt(num, private_key, n) for num in numbers]
